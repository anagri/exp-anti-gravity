...
function chunkDocument(content: string): Array<{ content: string; heading?: string }> {
  ...
  return chunks.length > 0
    ? chunks
    : [{ content: content.substring(0, chunkSize), heading: extractHeading(content) }];
}
...
async function uploadDocument(params: UploadDocumentParams): Promise<{ id: string }> {
  ...
  const id = uuidv4();
  await db.query(
    `INSERT INTO documents (id, filename, content, file_size, mime_type) VALUES (...)`
  );

  // Create indexing job (only if feature enabled)
  if (indexingEnabled) {
    await db.query(
      `INSERT INTO indexing_queue (id, document_id, status) VALUES ($1, $2, 'pending')`,
      [uuidv4(), id]
    );
  }
...
await worker.onProgress(
  Comlink.proxy((progress: IndexingProgress) => {
